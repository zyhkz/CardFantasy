...
public class AncientSummon {
    public static void apply(SkillResolver resolver, SkillUseInfo skillUseInfo, CardInfo attackCard, Player defenderHero,int victimCount,int effectNumber) throws HeroDieSignal {
        ...
        CardStatusItem statusItem = CardStatusItem.ancientSummon(skillUseInfo);
        ...
        for (CardInfo victim : victims) {
            ...
            if(effectNumber>0)
            {
                if(!victim.getStatus().getStatusOf(CardStatusType.海啸).isEmpty()){
                    victim.removeForce(CardStatusType.海啸);
                }
            }
            ...
            if (spreadList.size() > 0) {
                ...
                for (CardInfo randomVictim : randomVictims) {
                    ...
                    if (effectNumber > 0) {
                        if (!randomVictim.getStatus().getStatusOf(CardStatusType.海啸).isEmpty()) {
                            randomVictim.removeForce(CardStatusType.海啸);
                        }
                    }
    ...
    public static void explode(SkillResolver resolver, CardInfo deadCard, OnDamagedResult onDamagedResult, String... summonedCardsDescs) throws HeroDieSignal {
        ...
        GameUI ui = resolver.getStage().getUI();
        List<CardStatusItem> statusItems = deadCard.getStatus().getStatusOf(CardStatusType.海啸);
