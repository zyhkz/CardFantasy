...
public class GrudgeHp {
    public static void apply(SkillResolver resolver, SkillUseInfo skillUseInfo, CardInfo attackCard, Player defenderHero,int victimCount) throws HeroDieSignal {
        ...
        {
            if(fieldCard.getStatus().containsStatus(CardStatusType.咒皿))
            {
        ...
        CardStatusItem statusItem2 = CardStatusItem.grudgeHp(damage,skillUseInfo);
        ...
        for (CardInfo victim : victims) {
            ...
            if(effectNumber>0)
            {
                if(!victim.getStatus().getStatusOf(CardStatusType.咒皿).isEmpty()){
                    victim.removeForce(CardStatusType.咒皿);
            ...
            if (spreadList.size() > 0) {
                ...
                for (CardInfo randomVictim : randomVictims) {
                    ...
                    if (effectNumber > 0) {
                        if (!randomVictim.getStatus().getStatusOf(CardStatusType.咒皿).isEmpty()) {
                            randomVictim.removeForce(CardStatusType.咒皿);
    ...
    public static void Infected(SkillResolver resolver, CardInfo defendCard) throws HeroDieSignal {
        ...
        List<CardStatusItem> items = defendCard.getStatus().getStatusOf(CardStatusType.咒皿);
