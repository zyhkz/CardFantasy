...
public class Ancient {
    public static void apply(SkillResolver resolver, SkillUseInfo skillUseInfo, EntityInfo attacker, Player defenderHero,int victimCount,int effectNumber) throws HeroDieSignal {
        ...
        CardStatusItem statusItem = CardStatusItem.ancient(99999,skillUseInfo);
        ...
        for (CardInfo victim : victims) {
            ...
            if(effectNumber>0)
            {
                if(!victim.getStatus().getStatusOf(CardStatusType.远古).isEmpty()){
                    victim.removeForce(CardStatusType.远古);
                }
            }
            ...
            if(spreadList.size()>0){
                ...
                for (CardInfo randomVictim : randomVictims) {
                    ...
                    if (effectNumber > 0) {
                        if (!randomVictim.getStatus().getStatusOf(CardStatusType.远古).isEmpty()) {
                            randomVictim.removeForce(CardStatusType.远古);
                        }
                    }
