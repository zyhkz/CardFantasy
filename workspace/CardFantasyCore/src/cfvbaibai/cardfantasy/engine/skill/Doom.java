...
public class Doom {
    public static void apply(SkillResolver resolver, SkillUseInfo skillUseInfo, EntityInfo attacker, Player defenderHero,int victimCount,int effectNumber) throws HeroDieSignal {
        ...
        CardStatusItem statusItem = CardStatusItem.doom(skillUseInfo);
        ...
        for (CardInfo victim : victims) {
            ...
            if(effectNumber>0)
            {
                if(!victim.getStatus().getStatusOf(CardStatusType.厄运).isEmpty()){
                    victim.removeForce(CardStatusType.厄运);
                }
            }
